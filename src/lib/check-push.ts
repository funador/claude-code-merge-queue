 ...
 *
 * `land.ts` sets CLAUDE_CODE_MERGE_QUEUE_LANDING=1 right before its own push — that's
 ...
 * CLAUDE_CODE_MERGE_QUEUE_EMERGENCY_PUSH=1 and push. One env var, no prompts, no second
 * factor to remember — the same trust model as CLAUDE_CODE_MERGE_QUEUE_LANDING=1 above
...
function emergencyConfirmed(env: NodeJS.ProcessEnv): boolean {
  return env.CLAUDE_CODE_MERGE_QUEUE_EMERGENCY_PUSH === "1";
}

export function checkPush(
  refUpdates: RefUpdate[],
  cfg: Pick<ClaudeCodeMergeQueueConfig, "integrationBranch" | "productionBranch" | "protectedBranches">,
    ...
    if (protectedRefs.has(remoteRef) && !emergencyConfirmed(env)) {
      return {
        ...
        message: [
          ...
          `   Emergency override:  CLAUDE_CODE_MERGE_QUEUE_EMERGENCY_PUSH=1 git push …`,
          "",
        ].join("\n"),
      };
    }
    if (remoteRef === integrationRef && env.CLAUDE_CODE_MERGE_QUEUE_LANDING !== "1" && !emergencyConfirmed(env)) {
      return {
        ok: false,
        message: [
          "",
          `✋ Direct pushes to '${cfg.integrationBranch}' are blocked — landing goes through the queue.`,
          `   Land your work:  claude-code-merge-queue land`,
          `   Genuine emergency:  CLAUDE_CODE_MERGE_QUEUE_EMERGENCY_PUSH=1 git push …`,
          "",
        ].join("\n"),
      };
    }
