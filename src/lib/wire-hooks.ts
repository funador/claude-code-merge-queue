...
const HOOK_COMMAND = "npx localmerge hook worktree-create";
const PRE_PUSH_MARKER = "localmerge check-push";
export const PREFLIGHT_FILENAME = "localmerge-preflight.mjs";

const PACKAGE_SCRIPTS: Record<string, string> = {
  land: "localmerge land",
  sync: "localmerge sync",
  promote: "localmerge promote",
  preview: "localmerge preview",
  "preview:restore": "localmerge preview --restore",
  ...
  // instead of just being another `localmerge` subcommand.
...
export function wireHuskyPrePush(root: string): WireResult {
  ...
  const marker = "# --- LocalMerge (appended by `localmerge init`) — see node_modules/localmerge/hooks/pre-push for the full comments ---";
 ...
 * uncontested. Since LocalMerge is the one promising "pushes are gated now,"
 ...
 * localmerge happened once already), any lane that hasn't rebased past that
 ...
 * inside `localmerge land` can catch that; the binary it would need to run
 * to catch it is the very thing that's missing.
 *
 * That's why this has to be a plain, standalone script committed into the
 * CONSUMER repo rather than another `localmerge` subcommand: it must still
 * work the next time the tool itself gets renamed, so it can never invoke
 * `localmerge` (or import from the `localmerge` package) itself. It only
...
// Generated by \`localmerge init\` (wirePreflightScript) — do not hand-edit;
// re-run \`localmerge init\` after changing integrationBranch instead.
//
// Runs as "preland"/"presync" (npm's automatic pre<script> hook) before
// "land"/"sync". Deliberately self-contained — no import of localmerge
// itself — so it still catches a stale branch even across a future rename
// of this very tool. See wirePreflightScript in localmerge's source for why.
import { execFileSync } from "node:child_process";
