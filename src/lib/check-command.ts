 ...
 */
export function runCheckCommand(cfg: Pick<LocalMergeConfig, "checkCommand" | "checksRequired">, root: string): number {
  if (!cfg.checkCommand) {
    if (cfg.checksRequired) {
      console.error([
        ...
        "   Set checkCommand in localmerge.config.mjs, e.g. \"npm run check\".",
    ...
    console.log("localmerge check-push: no checkCommand configured (checksRequired: false — running with no checks, on purpose).");
    return 0;
  }

  console.log(`localmerge check-push: running "${cfg.checkCommand}"…`);
