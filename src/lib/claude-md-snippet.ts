 ...
 * remember to type `localmerge land` after every change, this snippet tells
 ...
 *
 * `localmerge init` writes this into CLAUDE.md (creating it if missing,
...
export const MARKER = "<!-- localmerge:workflow -->";

export function claudeMdSnippet(cfg: LocalMergeConfig): string {
  const promoteBlock = cfg.productionBranch
    ? `- **\`${cfg.productionBranch}\` is production, and it is not your call.** Only a human runs \`localmerge promote\` (${cfg.integrationBranch} → ${cfg.productionBranch}), on their own schedule. Never run it yourself, never suggest running it as part of "finishing" a task — landing on \`${cfg.integrationBranch}\` is the finish line for you.`
    : `- There's no separate production branch configured — \`${cfg.integrationBranch}\` already is it. Landing is the whole deal.`;

  return `${MARKER}
## LocalMerge workflow

This repo uses [LocalMerge](https://github.com/funador/localmerge) to run parallel Claude Code agents without them racing each other. It's not a suggestion — follow it by default, every session, without being asked:

- **You're probably already in a lane.** If you were started with \`claude --worktree <name>\`, LocalMerge's \`WorktreeCreate\` hook already isolated you into your own git worktree and branch. Just work.
- **Land the moment your change is green — don't ask first.** Run \`localmerge land\` yourself once tests/build/lint pass. This is pre-authorized, standard workflow, not a decision that needs sign-off. It rebases onto \`${cfg.integrationBranch}\`, pushes through the FIFO landing queue, and syncs the dev checkout — that whole sequence is "done," not "pushed to a branch and waiting."
- **Never push \`${cfg.integrationBranch}\` directly.** \`git push origin HEAD:${cfg.integrationBranch}\` is rejected by the pre-push hook anyway — always go through \`localmerge land\`.
- **A failed \`localmerge land\` is not a stopping point — landed is the only "done."** A rebase conflict aborts safely and leaves your working tree clean; that's expected, not an error to report and hand back. Resolve the conflicting diff yourself the same way you'd fix any other bug, \`git add\` the result, \`git rebase --continue\`, then re-run \`localmerge land\` — repeat until it actually lands. Don't say a change is finished, complete, or ready while it's still sitting unlanded in your lane; \`checkCommand\` still gates the real push regardless of how many attempts it took, so a wrong resolution gets caught there, not by a human reading it first.
- **Use \`localmerge build-lock -- <cmd>\` for anything heavy** (a full build, a slow test suite) so parallel lanes take turns instead of thrashing the machine together.
${promoteBlock}
`;
}
