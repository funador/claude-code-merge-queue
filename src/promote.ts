 ...
 *
 * This is the one command in LocalMerge that's deliberately NOT part of the
 * automated workflow. Agents land on `integrationBranch` continuously and
 * autonomously (see the CLAUDE.md workflow section `localmerge init` writes) —
 * production only moves when a human decides to run this. If your
 * localmerge.config has no `productionBranch` set, there's nothing to
 * promote: `integrationBranch` already IS production, and this is a no-op.
 *
 *   Usage:  localmerge promote   (run from anywhere in the repo)
 *
...
export async function promote(): Promise<number> {
  if (!hasConfig()) {
    console.error("localmerge promote: no localmerge.config found at the repo root.");
    return 1;
  }
  const cfg = await loadConfig();
  if (!cfg.productionBranch) {
    console.log(`localmerge promote: no productionBranch configured — '${cfg.integrationBranch}' already IS production. Nothing to do.`);
    return 0;
  }
  ...
  if (!prod.ok || !integ.ok) {
    console.error(`localmerge promote: could not resolve origin/${productionBranch} or origin/${integrationBranch} — are both branches created and fetched?`);
    return 1;
  }

  if (prod.out === integ.out) {
    console.log(`localmerge promote: ${productionBranch} already at ${integrationBranch} (${integ.out.slice(0, 7)}) — nothing to ship.`);
    return 0;
  }
  ...
  if (!ff.ok) {
    console.error(
      `localmerge promote: origin/${productionBranch} has commits NOT on origin/${integrationBranch} — history has diverged.\n` +
  ...
  if (!push.ok) {
    console.error(`localmerge promote: push to ${productionBranch} FAILED — production NOT updated.\n${push.out}`);
    return 1;
  }

  console.log(`localmerge promote: shipped ${integrationBranch} → ${productionBranch}  ${prod.out.slice(0, 7)} → ${integ.out.slice(0, 7)}`);
  return 0;
}
