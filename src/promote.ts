 ...
 *
 * This is the one command in Claude Code Merge Queue that's deliberately NOT part of the
 * automated workflow. Agents land on `integrationBranch` continuously and
 * autonomously (see the CLAUDE.md workflow section `claude-code-merge-queue init` writes) —
 * production only moves when a human decides to run this. If your
 * claude-code-merge-queue.config has no `productionBranch` set, there's nothing to
 * promote: `integrationBranch` already IS production, and this is a no-op.
 *
 *   Usage:  claude-code-merge-queue promote   (run from anywhere in the repo)
 *
