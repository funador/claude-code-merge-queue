 ...
 * after a push that ITSELF introduced or changed localmerge.config.mjs: the
 ...
 * `localmerge sync` (no caller-provided config) still loads fresh from
 * MAIN, same as before.
 */
export async function sync(providedCfg?: LocalMergeConfig): Promise<number> {
  let MAIN: string;
  try {
    MAIN = resolveMainCheckout(process.cwd());
  } catch {
    console.error("localmerge sync: not inside a git repo — nothing to do.");
    return 0;
  }
  ...
  if (!branch || branch === "HEAD") {
    console.error("localmerge sync: the checkout is detached or unresolved — left untouched.");
    return 0;
  }
  ...
  if (branch !== cfg.integrationBranch) {
    console.error(
      `localmerge sync: this checkout is on '${branch}', not the configured integrationBranch ` +
  ...
  if (!res.ok && /would be overwritten by merge/i.test(res.out)) {
    ...
    if (blocking.length === 0 && files.length > 0) {
      ...
      console.error(`localmerge sync: ${branch} has local changes blocking fast-forward (${blocking.join(", ")}). Left untouched — resolve in the checkout.`);
      return 0;
    }
  }
  ...
  if (/Not possible to fast-forward|diverging|non-fast-forward/i.test(res.out)) {
    console.error(`localmerge sync: local ${branch} has DIVERGED from ${upstream} (something was committed directly on the checkout). Left untouched — reconcile it manually.`);
    return 0;
  }

  console.error(`localmerge sync: could not fast-forward ${branch} — left untouched.\n${res.out.trim()}`);
  return 0;
}
