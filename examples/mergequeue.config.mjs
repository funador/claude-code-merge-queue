// mergequeue.config.mjs — lives at your repo root. `mergequeue init` writes a
...
export default {
  ...
  // The branch `mergequeue land` rebases onto and pushes to. Agents land
  // here continuously and autonomously — see the CLAUDE.md workflow section
  // `mergequeue init` writes.
  ...
  // `mergequeue promote`. null (the default) means integrationBranch IS
  ...
  // Build-output dirs `mergequeue preview` never copies onto your dev
  ...
  // build. `mergequeue init` tries to detect this from package.json
