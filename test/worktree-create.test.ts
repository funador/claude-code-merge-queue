...
function makeScratchRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), "localmerge-wt-test-"));
...
test("createLane bases a new lane on the main checkout's own HEAD, not a stale origin/integrationBranch", () => {
  const mainTop = makeScratchRepo();
  const remote = mkdtempSync(join(tmpdir(), "localmerge-wt-remote-"));
  try {
    ...
    // A commit made locally but never pushed — e.g. `localmerge init`'s own
    // wiring commit, right after Quickstart, before anyone has pushed it.
    execFileSync("node", ["-e", "require('fs').writeFileSync('localmerge.config.mjs', 'x')"], { cwd: mainTop });
    ...
    assert.ok(existsSync(join(wt, "localmerge.config.mjs")), "new lane must inherit the unpushed local commit, not fall back to stale origin/main");
  } finally {
    cleanupRepoAndLanes(mainTop);
...
test("isEphemeralNpxCopy detects npm's npx cache path and nothing else", () => {
  assert.equal(isEphemeralNpxCopy("/Users/jesse/.npm/_npx/abc123/node_modules/localmerge/dist/hooks/worktree-create.js"), true);
  assert.equal(isEphemeralNpxCopy("/Users/jesse/Desktop/projects/hola/node_modules/localmerge/dist/hooks/worktree-create.js"), false);
  assert.equal(isEphemeralNpxCopy("/Users/jesse/Desktop/projects/hola-2/node_modules/localmerge/dist/hooks/worktree-create.js"), false);
  ...
  assert.equal(
    isEphemeralNpxCopy("/Users/jesse/project/node_modules/.pnpm/localmerge@0.1.3/node_modules/localmerge/dist/hooks/worktree-create.js"),
    false,
  );
});

test("expectsLocalInstall is true only when the host's own package.json actually declares localmerge", () => {
  const dir = mkdtempSync(join(tmpdir(), "localmerge-expects-"));
  try {
    assert.equal(expectsLocalInstall(dir), false, "no package.json at all — e.g. a Haskell/Rust/Lua repo");

    writeFileSync(join(dir, "package.json"), JSON.stringify({ name: "x" }));
    assert.equal(expectsLocalInstall(dir), false, "package.json exists but never mentions localmerge");

    writeFileSync(join(dir, "package.json"), JSON.stringify({ devDependencies: { localmerge: "^0.1.7" } }));
    assert.equal(expectsLocalInstall(dir), true, "declared as a devDependency — a local install is expected");

    writeFileSync(join(dir, "package.json"), JSON.stringify({ dependencies: { localmerge: "^0.1.7" } }));
...
test("the hook refuses to run and fails loud when invoked from npx's ephemeral cache instead of the project's real install", async () => {
  // Reproduces the actual production incident: node_modules/localmerge was
  // missing/mid-upgrade, .claude/settings.json's `npx localmerge hook
  ...
  // install — declare localmerge as a devDependency, matching hola's real
  // package.json at the time of the incident.
  writeFileSync(join(mainTop, "package.json"), JSON.stringify({ devDependencies: { localmerge: "^0.1.3" } }));
  execFileSync("git", ["add", "-A"], { cwd: mainTop });
  execFileSync("git", ["commit", "-q", "-m", "add package.json"], { cwd: mainTop });
  const npxSimRoot = mkdtempSync(join(tmpdir(), "localmerge-npxsim-"));
  const copyRoot = join(npxSimRoot, "_npx", "deadbeef1234", "node_modules", "localmerge");
  try {
...
test("a non-Node host repo (no package.json at all) can still create a lane from npx's ephemeral cache — that's the only way it can ever run, not a broken install", async () => {
  ...
  // localmerge INTO, so npx's ephemeral cache is the only way it can ever
  // run localmerge commands. The guard above must not treat that as a
  ...
  const npxSimRoot = mkdtempSync(join(tmpdir(), "localmerge-npxsim-nonnode-"));
  const copyRoot = join(npxSimRoot, "_npx", "deadbeef5678", "node_modules", "localmerge");
  try {
    mkdirSync(copyRoot, { recursive: true });
    writeFileSync(join(copyRoot, "package.json"), JSON.stringify({ type: "module" }));
    cpSync(join(REPO_ROOT, "src"), join(copyRoot, "src"), { recursive: true });
